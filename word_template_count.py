...
def uniq_words(q):
    q = [x.replace(". "," ").replace(", "," ").replace("?"," ") for x in q]
    wrds = ' '.join([x.lower() for x in q]).split(" ")
    wrds = [x for x in wrds if x not in fwords]
